...
float sBoxS(in vec2 p, in vec2 b, in float sf) { return length(max(abs(p) - b + sf, 0.)) - sf; }
vec4 blocks(vec3 q3) {
...
float softShadow(vec3 ro, vec3 lp, vec3 n, float k) {
	...
	for (int i = min(iFrame, 0); i < maxIterationsShad; i++) {
		float d = map(ro + rd * t); shade = min(shade, k * d / t); t += clamp(d, .01, .25);
